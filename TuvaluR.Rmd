...
Y_causal[1] = 1  
for (i in 2:232) {
  Y_causal[i] <- 0.1 + 4*wt_el_trend[i] + 0.5 * Y_causal[i-1] + 2*err[i-1] + err[i]
}

arimax_causal <- auto.arima(Y_causal, xreg=wt_el_trend)
arimax_causal_inverse <- auto.arima(wt_el_trend, xreg=Y_causal)

coeftest(arimax_causal) #ARIMA(1,0,2)
coeftest(arimax_causal_inverse) #ARIMA(0,1,0)(0,0,1)[12]
arimax_causal
...
arimax_s_w_fit <- auto.arima(wt_el_trend, xreg=sl_el_trend)
...
```{r}
arimax_a_s_fit <- auto.arima(sl_el_trend, xreg=at_el_trend)
arimax_s_a_fit <- auto.arima(at_el_trend, xreg=sl_el_trend)

## ARIMAX: ARIMA(0,0,2)(0,0,1)[12]
coeftest(arimax_a_s_fit)
coeftest(arimax_s_a_fit) ## ARIMA(1,1,0)(2,0,1)[12]
...
arimax_aw_s_fit <- auto.arima(sl_el_trend, xreg=awt_el_trend)
